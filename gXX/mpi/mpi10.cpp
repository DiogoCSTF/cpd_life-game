...
void gen_generation(int*** grid, int*** new_grid, int id, int p, int** maximum, int epoch, long long N)
{
    //TOOOOOOOOOOOODOOOOOOOOOOOOOOOOOOOO   TIRAR ISTO DAQUI E TRAZER JA FEITO
    ...
    for (int x = 1; x < size+1; x++) {
        ...
        if(x==1){
            int node = (id+1)%p;
            for (int i = 0; i< N;i++) {
                MPI_Isend(new_grid[size][i], N, MPI_INT, node, N+i, MPI_COMM_WORLD, &request2[i]);
                MPI_Irecv(new_grid[size+1][i], N, MPI_INT, node, i, MPI_COMM_WORLD, &request4[i]);    
            }
        } else if (x == 2) {
            int node =  (id + p - 1) % p;
            for (int i = 0; i< N;i++) {
                MPI_Isend(new_grid[1][i], N, MPI_INT, node, i, MPI_COMM_WORLD, &request1[i]);
                MPI_Irecv(new_grid[0][i], N, MPI_INT, node, N+i, MPI_COMM_WORLD, &request3[i]);        
            }
        }
...
void full_generation(int*** grid1, int*** grid2, int id, int p, int** maximum, int gens, long long N, float density, int seed){
    bool grid_to_use = true;

    for (int x = 0; x < gens; x++) {
        if(grid_to_use){
            gen_generation(grid1, grid2, id, p, maximum, x, N);
            grid_to_use = false;
        }
        else {
            gen_generation(grid2, grid1, id, p, maximum, x, N);
            grid_to_use = true;
        }
        if (!id){
            cout << x << endl;
...
int main(int argc, char *argv[])
{
    ...
    //cout << (id-1)%p << " " << id << endl;
    for (int i = 0; i< N;i++) {
        MPI_Isend(grid1[1][i], N, MPI_INT, (id + p - 1) % p, i, MPI_COMM_WORLD, &request1[i]);
        MPI_Isend(grid1[size][i], N, MPI_INT, (id+1)%p, N+i, MPI_COMM_WORLD, &request2[i]);
        MPI_Irecv(grid1[0][i], N, MPI_INT, (id + p - 1) % p, N+i, MPI_COMM_WORLD, &request3[i]);        
        MPI_Irecv(grid1[size+1][i], N, MPI_INT, (id+1)%p, i, MPI_COMM_WORLD, &request4[i]);    
    }
    for (int i = 0; i< N;i++) {
        MPI_Wait(&request3[i], MPI_STATUS_IGNORE);
        MPI_Wait(&request4[i], MPI_STATUS_IGNORE);
    ...
    exec_time = - MPI_Wtime();
    full_generation(grid1, grid2, id, p, maximum, atoi(argv[1]), N, atof(argv[3]), atoi(argv[4]));
    exec_time += MPI_Wtime();
    if(!id){
        fprintf(stderr, "%.1fs\n", exec_time);
        fflush(stderr);
