...
void gen_generation(int*** grid, int*** new_grid, int id, int p, int** maximum, MPI_Request** requests, int epoch, long long N, bool flag)
{
    int size;
    double div = (float) N/p;
    int val1 = (flag) ? 4 : 0;
    ...
    for (int x = 1; x < size+1; x++) {
        ...
        if(x==1){
            int node = (id+1)%p;
            for (int i = 0; i< N;i++) {
                MPI_Isend(new_grid[size][i], N, MPI_INT, node, N+i, MPI_COMM_WORLD, &requests[1+val1][i]);
                MPI_Irecv(grid[size+1][i], N, MPI_INT, node, i, MPI_COMM_WORLD, &requests[3+val2][i]);    
            }
        } else if (x == 2) {
            int node =  (id + p - 1) % p;
            for (int i = 0; i< N;i++) {
                MPI_Isend(new_grid[1][i], N, MPI_INT, node, i, MPI_COMM_WORLD, &requests[0+val1][i]);
                MPI_Irecv(grid[0][i], N, MPI_INT, node, N+i, MPI_COMM_WORLD, &requests[2+val2][i]);        
            }
        }
    }
...
void full_generation(int*** grid1, int*** grid2, int id, int p, int** maximum , MPI_Request** requests, int gens, long long N, float density, int seed){
    ...
    gen_generation(grid1, grid2, id, p, maximum, requests, 0, N, grid_to_use);
    grid_to_use = false;
    ...
    for (int x = 1; x < gens; x++) {
        if(grid_to_use){
            ...
            gen_generation(grid1, grid2, id, p, maximum, requests, x, N, grid_to_use);
            grid_to_use = false;
        }
        else {
            for (int i = 0; i< N;i++) {
                MPI_Wait(&requests[6][i], MPI_STATUS_IGNORE);
                MPI_Wait(&requests[7][i], MPI_STATUS_IGNORE);
            }

            for (int i = 0; i< N;i++) {
                MPI_Wait(&requests[4][i], MPI_STATUS_IGNORE);
                MPI_Wait(&requests[5][i], MPI_STATUS_IGNORE);
            }
            gen_generation(grid2, grid1, id, p, maximum, requests, x, N, grid_to_use);
            grid_to_use = true;
        }
        if (!id){
            cout << x << endl;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 8; i++) { // 2env 2rec grid1 2env 2rec grid2
    ...
    if(atoi(argv[1])>1){
        for (int i = 0; i< N;i++) {
            MPI_Irecv(grid2[0][i], N, MPI_INT, (id + p - 1) % p, N+i, MPI_COMM_WORLD, &requests[6][i]);        
            MPI_Irecv(grid2[size+1][i], N, MPI_INT, (id+1)%p, i, MPI_COMM_WORLD, &requests[7][i]);    
        }
    }
