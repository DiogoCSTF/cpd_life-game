...
void gen_generation(int*** grid, int*** new_grid, int** upper, int ** lower, int id, int p, int** maximum, int epoch, long long N)
{
    ...
    for (int x = 0; x < size; x++) {
        ...
        for (int y = 0; y < N; y++) {
            ...
            for (int z = 0; z < N; z++) {
                ...
                if (species != 0)
                    max[species-1]++;
                if (x == 1) {
                    new_grid[xi][y][z] = visit_node(grid, upper, true, N, xi, y, z);
                } else if (!x) {
                    new_grid[xi][y][z] = visit_node(grid, lower, false, N, xi, y, z);
                }else {
        ...
        if(!x){
            int node = (id+1)%p;
            for (int i = 0; i< N;i++) {
                MPI_Isend(new_grid[size-1][i], N, MPI_INT, node, N+i, MPI_COMM_WORLD, &request2[i]);
                MPI_Irecv(lower[i], N, MPI_INT, node, i, MPI_COMM_WORLD, &request4[i]);    
            }
        } else if (x == 1) {
            int node =  (id + p - 1) % p;
            for (int i = 0; i< N;i++) {
                MPI_Isend(new_grid[0][i], N, MPI_INT, node, i, MPI_COMM_WORLD, &request1[i]);
                MPI_Irecv(upper[i], N, MPI_INT, node, N+i, MPI_COMM_WORLD, &request3[i]);        
            }
        }
