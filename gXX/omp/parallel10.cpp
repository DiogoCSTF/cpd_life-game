...
// Visits node
int visit_node(int*** grid, long long N, int x, int y, int z, int* xi, int* yi, int* zi, int* xii, int* yii, int* zii) {
    fill_vector(xi,x,N);
    fill_vector(yi,y,N);
    fill_vector(zi,z,N);
    ...
    for (; *xii < 3; (*xii)++) {
        *yii = 0;
        for (; *yii < 3; (*yii)++) {
            *zii = 0;
            for (; *zii < 3; (*zii)++) {
                int species = grid[xi[*xii]][yi[*yii]][zi[*zii]];
                if (species != 0){
                    max[species-1]++;
                    sum++;
                }    
        ...
        for (int x = 0; x < N; x++) {
            for (int y = 0; y < N; y++) {
                for (int z = 0; z < N; z++) {
                    ...
                    if (species != 0)
                        max[species-1]++;
                    new_grid[x][y][z] = visit_node(grid, N, x, y, z, xi, yi, zi, &xii, &yii, &zii);
                }
            }
        }
    }
